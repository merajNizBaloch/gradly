...
function removeWhiteFromImage(dataUrl: string): Promise<string> { return new Promise((resolve) => { const image = new Image(); image.onload = () => { const canvas = document.createElement("canvas"); canvas.width = image.naturalWidth; canvas.height = image.naturalHeight; const ctx = canvas.getContext("2d"); if (!ctx) return resolve(dataUrl); ctx.drawImage(image, 0, 0); const pixels = ctx.getImageData(0, 0, canvas.width, canvas.height); for (let i = 0; i < pixels.data.length; i += 4) { const r = pixels.data[i], g = pixels.data[i + 1], b = pixels.data[i + 2]; if (r > 238 && g > 238 && b > 238) pixels.data[i + 3] = 0; } ctx.putImageData(pixels, 0, 0); resolve(canvas.toDataURL("image/png")); }; image.src = dataUrl; }); }
...
function applyProfileToPage(profile: Profile) {
  ...
  [profile.name, profile.motto, profile.address, profile.contact].forEach((value, index) => { const input = inputs[index]; if (!input) return; const setter = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")?.set; setter?.call(input, value); input.dispatchEvent(new Event("input", { bubbles: true })); input.dispatchEvent(new Event("change", { bubbles: true })); });
  const logo = document.querySelector<HTMLImageElement>("img[alt='School logo']");
  if (logo) { if (profile.logo) logo.src = profile.logo; logo.style.transform = `translate(${profile.x}%, ${profile.y}%) scale(${profile.zoom / 100})`; }
  const paper = document.querySelector<HTMLElement>(".gradly-paper");
  if (paper) { paper.style.backgroundImage = profile.logo ? `linear-gradient(rgba(255,255,255,${profile.removeWhite ? 0.72 : 0.90}),rgba(255,255,255,${profile.removeWhite ? 0.72 : 0.90})),url(\"${profile.logo}\")` : "none"; paper.style.backgroundPosition = `${50 + profile.x}% ${50 + profile.y}%`; paper.style.backgroundSize = `${48 * profile.zoom / 100}% auto`; paper.style.backgroundRepeat = "no-repeat"; }
...
export default function SidebarTabs() {
  ...
  useEffect(() => {
    const stored = loadProfile(); setProfile(stored); setDraft(stored); setSaved(Boolean(localStorage.getItem(STORAGE_KEY))); applyProfileToPage(stored);
    const sidebar = document.querySelector<HTMLElement>("section.no-print.space-y-4");
    if (sidebar) { const slot = document.createElement("div"); slot.setAttribute("data-gradly-sidebar-tabs-slot", "true"); sidebar.insertBefore(slot, sidebar.firstElementChild); setSidebarMount(slot); }
    const header = document.querySelector<HTMLElement>("header.no-print");
    if (header) { const slot = document.createElement("div"); slot.setAttribute("data-gradly-school-topbar-slot", "true"); slot.className = "order-2 flex items-center"; header.querySelector(":scope > div > div:last-child")?.appendChild(slot) || header.appendChild(slot); setTopMount(slot); }
    return () => { document.querySelector("[data-gradly-sidebar-tabs-slot]")?.remove(); document.querySelector("[data-gradly-school-topbar-slot]")?.remove(); };
  }, []);
