...
export default function GradlyWorkspaceLocal() {
  ...
  useEffect(() => {
    try {
      ...
      if (editReportId) {
        const result = getLocalResult(editReportId);
        if (!result) {
          ...
          setSaveMessage("Loaded from this browser.");
        }
      } else {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      saveLocalDraft({ school, design, student, father, roll, klass, session, exam, dob, attendance, position, photo, subjects, bands, teacherRemarks, principalRemarks });
    }, 250);
    return () => window.clearTimeout(timer);
  }, [hydrated, editReportId, school, design, student, father, roll, klass, session, exam, dob, attendance, position, photo, subjects, bands, teacherRemarks, principalRemarks]);
  ...
  const saveResult = () => {
    ...
    try {
      ...
      const existing = getLocalResult(reportId);
      ...
      clearLocalDraft();
      setSaveMessage("Saved in this browser. It will remain here until browser/site data is cleared.");
    } catch (error) {
  ...
  if (!hydrated) {
    return <main className="grid min-h-screen place-items-center bg-[#F4F8FC]"><div className="border border-[#D8E3F0] bg-white p-7 text-center"><img src="/gradly-logo.svg" alt="Gradly" className="mx-auto h-11 w-11" /><p className="mt-3 text-sm font-bold text-[#0B3477]">Opening Gradly…</p></div></main>;
  }
    ...
    <main className="min-h-screen bg-[#F4F8FC] text-[#17324D]">
      ...
      <div className="mx-auto grid max-w-[1800px] gap-4 px-4 py-4 sm:px-6 xl:h-[calc(100vh-75px)] xl:grid-cols-[210px_430px_minmax(0,1fr)] xl:px-8">
        ...
        <section className="no-print border border-[#D8E3F0] bg-white shadow-[0_18px_55px_rgba(15,74,168,.07)] xl:h-full xl:overflow-y-auto">
          ...
          <div className="p-5">
            {activeStep === "student" && <div className="space-y-5">
              <SectionIntro eyebrow="Student record" title="Student & examination" description="This draft is automatically kept in this browser while you work." />
              ...
              <button type="button" onClick={() => photoRef.current?.click()} className="flex w-full items-center gap-3 border border-dashed border-[#C9D8E7] bg-[#F8FBFF] p-4 text-left"><span className="grid h-12 w-10 place-items-center overflow-hidden border bg-white">{photo ? <img src={photo} alt="Student" className="h-full w-full object-cover" /> : <ImagePlus size={18} />}</span><span><span className="block text-xs font-black text-slate-700">{photo ? "Student photo added" : "Add student photo"}</span><span className="text-[10px] text-slate-400">Stored locally with the result</span></span></button>
            </div>}
            ...
            {activeStep === "finalize" && <div className="space-y-5">
              <SectionIntro eyebrow="Final actions" title="Save or download this result" description="Saving uses only this browser. Downloads are generated locally from the live card." />
              ...
              <div className="border border-[#D8E3F0] bg-[#F8FBFF] p-4">
                <div className="flex items-start gap-3"><HardDrive className="mt-0.5 text-[#0F4AA8]" size={18} /><div><p className="text-xs font-black text-slate-800">Browser save</p><p className="mt-1 text-[10px] leading-4 text-slate-500">No Supabase/database request is made. Saved results stay on this device/browser until its site data is cleared.</p></div></div>
                <button type="button" onClick={saveResult} className="mt-3 flex w-full items-center justify-center gap-2 bg-[#0F4AA8] px-4 py-3 text-sm font-black text-white"><HardDrive size={16} /> {savedId ? "Update browser save" : "Save to this browser"}</button>
              </div>
              ...
              <div className="border-t border-[#E4ECF5] pt-4"><p className="mb-2 text-[9px] font-black uppercase tracking-[.16em] text-slate-400">Download result</p><ExportActions student={student} design={design} /></div>
        ...
        <section className="min-h-[680px] overflow-hidden border border-[#D8E3F0] bg-[#EAF1F7] shadow-[0_18px_55px_rgba(15,74,168,.07)] xl:h-full">
          <div className="no-print flex items-center justify-between gap-3 border-b border-[#D8E3F0] bg-white px-4 py-3"><div><p className="text-[9px] font-black uppercase tracking-[.18em] text-[#11B8B2]">Live preview</p><p className="mt-0.5 text-xs font-bold text-slate-500">Design, remarks and paper size update instantly.</p></div><DesignDropdown value={design} onChange={setDesign} /></div>
          <ResultCard school={school} design={design} student={student} father={father} roll={roll} klass={klass} session={session} exam={exam} dob={dob} attendance={attendance} position={position} photo={photo} subjects={subjects} bands={bands} teacherRemarks={teacherRemarks} principalRemarks={principalRemarks} verificationUrl="" />
        </section>
      </div>
